...
**Linearizable reads cost ~1.5×** — that is the ReadIndex quorum round-trip,
...
consequence of the client, not a fluke — see below. Writes to stale reads
...
Group commit — the event loop drains every queued proposal before fsyncing
once (`crates/server/src/core.rs`) — is what makes the write path
...
`NotLeader`. A stale read never triggers that answer — every replica will
...
leader and clients re-route, then fully recovers to ~18k ops/s by t=12s — 0 of
